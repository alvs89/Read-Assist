...
for (let i = 1; i <= numRows; i++) {
  ...
  // Alternate modes so the validation file contains both pre-test and post-test rows.
  ...
  const passage = passages.find(entry => entry.gradeLevel === assigned_passage_grade && entry.language === language && entry.passageSet === passage_set)
    || passages.find(entry => entry.gradeLevel === assigned_passage_grade && entry.language === language)
    || passages.find(entry => entry.language === language)
    || passages[0];
  const total_words_of_passage = passage.wordCount;

  // Shape the generated data so the rule engine sees all three reading profiles.
  ...
  // Force a few edge rows so QA can confirm the profile thresholds and tie cases.
...
fs.writeFileSync('readassist_test_dataset_v3.csv', csvContent);
console.log("CSV dataset generated successfully!");
