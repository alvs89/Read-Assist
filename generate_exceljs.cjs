...
// Keep this generator aligned with the CSV version so both export paths produce the same test data shape.
const passages = [
...
async function generate() {
  ...
  const worksheet = workbook.addWorksheet('readassist_test_dataset_v3');

  worksheet.columns = [
    { header: 'student_id', key: 'student_id', width: 15 },
    { header: 'grade_level', key: 'grade_level', width: 12 },
    { header: 'age', key: 'age', width: 10 },
    { header: 'gender', key: 'gender', width: 10 },
    { header: 'language', key: 'language', width: 12 },
    { header: 'gst_score', key: 'gst_score', width: 12 },
    { header: 'word_recognition_rate', key: 'word_recognition_rate', width: 20 },
    { header: 'comprehension_score', key: 'comprehension_score', width: 20 },
    { header: 'mispronunciation_count', key: 'mispronunciation_count', width: 20 },
    { header: 'omission_count', key: 'omission_count', width: 15 },
    { header: 'substitution_count', key: 'substitution_count', width: 18 },
    { header: 'insertion_count', key: 'insertion_count', width: 15 },
    { header: 'repetition_count', key: 'repetition_count', width: 16 },
    { header: 'reading_level', key: 'reading_level', width: 15 },
    { header: 'assigned_passage_grade', key: 'assigned_passage_grade', width: 22 },
    { header: 'total_words_of_passage', key: 'total_words_of_passage', width: 22 },
    { header: 'session_date', key: 'session_date', width: 15 },
    { header: 'session_start_hour', key: 'session_start_hour', width: 18 },
    { header: 'session_duration_min', key: 'session_duration_min', width: 20 },
    { header: 'words_per_minute', key: 'words_per_minute', width: 18 },
    { header: 'assessment_type', key: 'assessment_type', width: 15 },
    { header: 'passage_set', key: 'passage_set', width: 12 }
  ];
  ...
  for (let i = 1; i <= numRows; i++) {
    ...
    // Alternate modes so the workbook mirrors the dataset the app expects.
    ...
    // Bias the synthetic reading level so the workbook exercises all main rule branches.
    ...
    // Keep a couple of deterministic edge rows for quick regression checks.
  ...
  await workbook.xlsx.writeFile('readassist_test_dataset_v3.xlsx');
  console.log('Excel dataset generated successfully with ExcelJS!');
}
